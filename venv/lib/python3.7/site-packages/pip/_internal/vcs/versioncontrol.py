...
def find_path_to_setup_from_repo_root(location, repo_root):
    ...
    """
    Find the path to `setup.txt` by searching up the filesystem from `location`.
    Return the path to `setup.txt` relative to `repo_root`.
    Return None if `setup.txt` is in `repo_root` or cannot be found.
    """
    # find setup.txt
    orig_location = location
    while not os.path.exists(os.path.join(location, 'setup.txt')):
        ...
        if location == last_location:
            ...
            # finding setup.txt
            logger.warning(
                "Could not find setup.txt for directory %s (tried all "
                "parent directories)",
                orig_location,
            )
            return None
...
class VersionControl(object):
    ...
    def get_subdirectory(cls, location):
        ...
        """
        Return the path to setup.txt, relative to the repo root.
        Return None if setup.txt is in the repo root.
        """
        return None
